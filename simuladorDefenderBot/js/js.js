...
simular = function (){
    var simuls = prompt("quantas simulações?",200);
    $.ui.showMask();
	  ...
	  for(var i=0;i<nslots;i++){
	    //ciclo dos ataques
		  if(inimigos[i].prioridade==p){
              a=0;
              para=false;
		  while(a<3 && !para){ // for(var a=0;a<3;a++){
...
cura1=function(){
     if(energiaBot>=consumoCura1){
      ...
      //  $.ui.popup("Não tem energia suficiente para a cura 1")
    }
    return;
}
cura2=function(){
    ...
    //    $.ui.popup("Não tem energia suficiente para a cura 2")
    }
    return;
}
cura3=function(){
     ...
     //     actualizaVida();
      ...
      //  $.ui.popup("Não tem energia suficiente para a cura 3")
    }
    return;
}
...
/*rondaInimigo =function(){
var aux;
$("#ronda-inimigo").html('<div class="container" id="rondaInimigo'+turno+'" style="width: 100%";><h2>Turno '+turno+'</h2></div><a class="button" onclick="fimRonda()">Continuar</a>');

$.ui.loadContent("#ronda-inimigo",false,false,"slide");
  for(var i=1; i<=6;i++){
    if(inimigos[i].colocado){
       //o atacante na posicao i vai atacar se ainda tiver ataques
        if(inimigos[i].ataques>0){
          ...
          vidaBot-=inimigos[i].forca*aux; //robot perde vida devido ao ataque
          inimigos[i].ataques--;
        $("#ataques"+i).html(inimigos[i].ataques);
        $("#rondaInimigo"+turno).append('Um '+inimigos[i].name+' atacou com força de '+inimigos[i].forca*aux+'<br>');
          actualizaVida();
        }
     }
  }
...
/*fimRonda = function(){ 
    if(turno<6){
        ...
        if(vidaBot>0){ 
            $.ui.popup("<h1>Sobreviveu a esta ronda</h1>");
            ronda();
            }
        else{ 
            $.ui.popup("<h1>O Defender Bot foi derrotado</h1>");
            inicializa();
      ...
      if(vidaBot>0){ 
            $.ui.popup("<h1>Parabéns! Sobreviveu ate ao fim</h1>");
            inicializa();
            }
        else{ 
            $.ui.popup("<h1>O Defender Bot foi derrotado</h1>");
...
/*ronda=function(){
    turno++;
    $("#afui #content #tabela-info #turno").html(turno);
    $.ui.loadContent("#tabela",false,false,"slide");
  posiciona();
 actualizaVida();
    slot=1;
    acao();
...
/*actualizaVida = function(){
    $("#afui #content #tabela-info #vidaBot").html(vidaBot);
    $("#afui #content #tabela-info #energiaBot").html(energiaBot);
    return;
}*/
/*acao = function(){
    if(array[slot].colocado){
            $("#inimigo").html('<h2>Slot '+slot+': '+array[slot].name+'</h2>');
         }
    else{
        $("#inimigo").html('<h2>Slot '+slot+': Está vazio</h2>');
        }
       return;
}*/
/*proximoSlot = function(){
    if(slot<6)
        slot++;
    acao();
    return;
}
anteriorSlot = function(){
    if(slot>1)
        slot--;
    acao();
    return;
